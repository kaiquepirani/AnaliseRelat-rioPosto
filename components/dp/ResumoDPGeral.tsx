...
export default function ResumoDPGeral() {
  ...
  const [mostrandoLimpar, setMostrandoLimpar] = useState(false)
  ...
  const semFechamentos = fechamentos.length === 0
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      {/* ── ZONA DE PERIGO — botão limpar tudo (sempre visível) ── */}
      <div style={{ marginTop: '1rem', padding: '1rem 1.25rem', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#7f1d1d' }}>
              Remove TODOS os fechamentos, colaboradores e pagamentos do banco de dados. Use apenas para começar do zero após uma correção do parser.
            </div>
          </div>
          <button
            onClick={() => setMostrandoLimpar(true)}
            style={{
              padding: '0.6rem 1rem',
      ...
      {mostrandoLimpar && (
        <ModalLimparTudo
          onClose={() => setMostrandoLimpar(false)}
          onCompleto={async () => { setMostrandoLimpar(false); await carregar() }}
        />
      )}
    </div>
  )
...
// ─────────────────────────────────────────────────────────────────
// MODAL — LIMPAR TUDO (com confirmação por digitação)
// ─────────────────────────────────────────────────────────────────
function ModalLimparTudo({ onClose, onCompleto }: { onClose: () => void; onCompleto: () => Promise<void> | void }) {
  const PALAVRA_CONFIRMACAO = 'LIMPAR'
  const [texto, setTexto] = useState('')
  const [executando, setExecutando] = useState(false)
  const [progresso, setProgresso] = useState<string>('')
  const [erro, setErro] = useState('')

  const podeLimpar = texto.trim().toUpperCase() === PALAVRA_CONFIRMACAO
  ...
  const limpar = async () => {
    ...
    setErro('')
    try {
      setProgresso('Removendo colaboradores…')
      let res = await fetch('/api/dp/colaboradores', { method: 'PATCH' })
      if (!res.ok) throw new Error('Falha ao limpar colaboradores')

      setProgresso('Removendo fechamentos…')
      res = await fetch('/api/dp/fechamentos', { method: 'PATCH' })
      if (!res.ok) throw new Error('Falha ao limpar fechamentos')

      setProgresso('Removendo pagamentos…')
      res = await fetch('/api/dp/pagamentos', { method: 'PATCH' })
      if (!res.ok) throw new Error('Falha ao limpar pagamentos')
  ...
  return (
    <div
      ...
      style={{
        position: 'fixed', inset: 0, background: 'rgba(15,20,40,0.65)', backdropFilter: 'blur(4px)',
        zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 12,
        fontFamily: 'inherit',
      }}
    >
      <div
      ...
      >
        <div style={{ background: '#dc2626', color: 'white', padding: '18px 22px' }}>
          <div style={{ fontSize: 11, opacity: 0.85, letterSpacing: 0.5, fontWeight: 600 }}>AÇÃO IRREVERSÍVEL</div>
          <div style={{ fontSize: 18, fontWeight: 800, marginTop: 4 }}>
            🧹 Limpar todo o Departamento Pessoal
          </div>
        </div>

        <div style={{ padding: '20px 22px', flex: 1 }}>
          <div style={{ fontSize: 13, color: '#374151', lineHeight: 1.55, marginBottom: 16 }}>
            Esta ação remove permanentemente do banco de dados:
          </div>
          <ul style={{ fontSize: 13, color: '#374151', paddingLeft: 22, margin: '0 0 16px 0', lineHeight: 1.7 }}>
            <li>Todos os <strong>colaboradores cadastrados</strong></li>
            <li>Todos os <strong>fechamentos importados</strong> (todos os meses)</li>
            <li>Todos os <strong>pagamentos registrados</strong></li>
          </ul>
          <div style={{ background: '#fef3c7', border: '1px solid #fde68a', padding: '10px 12px', borderRadius: 8, fontSize: 12, color: '#92400e', marginBottom: 16, lineHeight: 1.5 }}>
            <strong>Use isso apenas se você quer recomeçar do zero.</strong> Os dados serão recriados automaticamente quando você reimportar as folhas Excel.
          </div>

          <label style={{ fontSize: 12, fontWeight: 700, color: '#374151', display: 'block', marginBottom: 6 }}>
            Para confirmar, digite <code style={{ background: '#fee2e2', color: '#991b1b', padding: '2px 6px', borderRadius: 4, fontWeight: 700 }}>{PALAVRA_CONFIRMACAO}</code> abaixo:
          </label>
          <input
            type="text"
            value={texto}
            ...
            style={{
              ...
              borderRadius: 8, fontSize: 14, fontWeight: 600, color: '#dc2626',
              fontFamily: 'inherit', outline: 'none',
              borderColor: podeLimpar ? '#dc2626' : '#e5e7eb',
            }}
          />
          ...
          {executando && progresso && (
            <div style={{ marginTop: 12, padding: '8px 12px', background: '#eff6ff', color: '#1d4ed8', borderRadius: 6, fontSize: 13, fontWeight: 600 }}>
              {progresso}
            </div>
          )}
        </div>

        <div style={{ borderTop: '1px solid #e5e7eb', background: '#f9fafb', padding: '14px 22px', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          <button
            onClick={onClose}
            disabled={executando}
            style={{
              padding: '10px 18px', border: '1px solid #e5e7eb', background: 'white',
              color: '#374151', borderRadius: 8, cursor: executando ? 'default' : 'pointer',
          ...
          <button
            onClick={limpar}
            disabled={!podeLimpar || executando}
            style={{
              padding: '10px 22px', border: 'none',
              background: !podeLimpar || executando ? '#9ca3af' : '#dc2626',
              color: 'white', borderRadius: 8,
              cursor: !podeLimpar || executando ? 'default' : 'pointer',
              fontWeight: 700, fontSize: 14, fontFamily: 'inherit',
            }}
          >
            {executando ? 'Limpando…' : 'Limpar tudo'}
          </button>
        </div>
      </div>
    </div>
