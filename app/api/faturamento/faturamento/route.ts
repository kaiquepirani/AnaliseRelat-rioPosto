...
// Normaliza um valor de célula para number ou null
const valorParaNumero = (v: any): number | null => {
  ...
  if (typeof v === 'string') {
    ...
    // Tenta extrair número (alguns campos têm texto tipo "FALTA R$201,50")
...
// Detecta se o nome da aba é um ano (2022, 2023, etc)
...
// Detecta se uma linha é a linha de TOTAL (deve ser ignorada)
...
export async function POST(req: NextRequest) {
  ...
  // Processa cada aba que seja um ano (2022, 2023, ..., ignora "Dashboard")
  for (const nomeAba of workbook.SheetNames) {
    ...
    for (let i = 1; i < dados.length; i++) {
      ...
      // Pega 12 valores das colunas 1..12 (jan a dez)
      const meses: (number | null)[] = []
      for (let j = 1; j <= 12; j++) {
        meses.push(valorParaNumero(linha[j]))
      }

      // Se todos os meses são null, ignora (linha vazia)
